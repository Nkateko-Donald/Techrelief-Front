...
export default function VotingSessionPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [settingsRes, nominationsRes, votesRes, usersRes] = await Promise.all([
  ...
  const hasUserVoted = useMemo(() => {
    return currentUser ? votes.some(vote => vote.VoterID === currentUser.UserID) : false;
  }, [votes, currentUser]);
  ...
  const votingStats = useMemo(() => {
    ...
    const participationRate = users.length > 0 
      ? Math.round((totalVoters / users.length) * 100) 
      : 0;
    return {
      totalVotes,
      totalVoters,
  ...
  const NominationDetailsModal = ({
    ...
    const hasVotedFor = currentUser ? votes.some(
      vote => vote.VoterID === currentUser.UserID && 
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full border border-slate-200 max-h-[90vh] overflow-y-auto">
          <div className="p-6">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-xl font-bold text-gray-800">Nomination Details</h3>
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                <div className="flex-shrink-0">
                  {nominee?.ProfilePhoto ? (
                    <img className="h-16 w-16 rounded-full" src={nominee.ProfilePhoto} alt={nominee.FullName} />
                  ) : (
                    <DefaultProfileIcon className="h-16 w-16" />
                  )}
                </div>
                <div>
                  <h4 className="text-lg font-semibold text-gray-800">{nominee?.FullName || "Unknown User"}</h4>
              ...
              <div className="flex items-center space-x-4">
                <div className="flex-shrink-0">
                  {nominator?.ProfilePhoto ? (
                    <img className="h-16 w-16 rounded-full" src={nominator.ProfilePhoto} alt={nominator.FullName} />
                  ) : (
                    <DefaultProfileIcon className="h-16 w-16" />
                  )}
                </div>
                <div>
                  <h4 className="text-lg font-semibold text-gray-800">{nominator?.FullName || "Unknown User"}</h4>
              ...
              <div>
                ...
                <div className={`mt-1 px-3 py-1 inline-flex text-sm font-medium rounded-full ${
                  nomination.Status === "Accepted" ? "bg-green-100 text-green-800" :
              ...
              <div>
                ...
                <p className="mt-1 text-gray-700">{nomination.Message || "No message provided"}</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h5 className="text-sm font-medium text-gray-500">NOMINATED AT</h5>
                  <p className="mt-1 text-gray-700">{formatDate(nomination.NominatedAt)}</p>
                </div>
                {nomination.RespondedAt && (
                  <div>
                    <h5 className="text-sm font-medium text-gray-500">RESPONDED AT</h5>
                    <p className="mt-1 text-gray-700">{formatDate(nomination.RespondedAt)}</p>
                  </div>
                )}
              </div>
              ...
              <div className="pt-4 border-t border-gray-200">
                <div className="flex justify-between items-center">
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-700 font-medium">{nomination.VoteCount} votes</span>
                  </div>
                  {!isCurrentUser && currentUser && (
                    ...
                    >
                      {hasVotedFor ? 'Vote Submitted' : hasUserVoted ? 'Already Voted' : 'Vote for Nominee'}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="card-header text-white py-4">
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <p className="mb-0 opacity-75">Control and monitor ongoing voting sessions</p>
              </div>
            ...
            <div className="p-4">
              ...
              <div className="mb-5">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <div className={`px-3 py-1 rounded-pill ${votingSettings.VotingEnabled ? 'bg-success bg-opacity-10 text-success' : 'bg-secondary bg-opacity-10 text-secondary'}`}>
                    <i className={`fas ${votingSettings.VotingEnabled ? 'fa-check-circle' : 'fa-pause-circle'} me-1`}></i>
                ...
                <div className="row g-3">
                  ...
                  <div className="col-md-4">
                    <div className="p-3 border rounded">
                      ...
                      <h5 className="mb-0">{formatDate(votingSettings.StartDate)}</h5>
                    </div>
                  </div>
                  ...
                  <div className="col-md-4">
                    <div className="p-3 border rounded">
                      ...
                      <h5 className="mb-0">{formatDate(votingSettings.EndDate)}</h5>
                    </div>
                  </div>
                </div>
                ...
                <div className="d-flex gap-2 mt-3">
                  ...
                  <button
                    ...
                    className={`btn ${votingSettings.VotingEnabled ? 'btn-danger' : 'btn-success'}`}
                  >
                    <i className={`fas ${votingSettings.VotingEnabled ? 'fa-stop' : 'fa-play'} me-2`}></i>
                    {votingSettings.VotingEnabled ? 'Stop Voting' : 'Start Voting'}
                  </button>
                </div>
              </div>
                            ...
                            <div className="card-body text-center">
                              ...
                              <div className="avatar mx-auto mb-3">
                                {nominee?.ProfilePhoto ? (
                                  ...
                                  <>{nominee?.FullName.charAt(0).toUpperCase() || 'U'}</>
                                )}
                              </div>
                              ...
                              <p className="text-muted mb-3">{nomination.VoteCount} votes</p>

                              {!isCurrentUser && currentUser && (
                                <button
                                  onClick={() => handleVote(nomination.NomineeID)}
                                  disabled={hasUserVoted || hasVotedFor}
                                  className={`btn btn-sm ${hasUserVoted || hasVotedFor ? 'btn-outline-secondary' : 'btn-primary'}`}
                                >
                                  {hasVotedFor ? (
                                    <>
              ...
              <div className="mb-5">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h4 className="fw-semibold">
                    ...
                    Nominations
                    <span className="badge bg-primary ms-2">{nominations.length}</span>
                  </h4>
                </div>
                          ...
                          <tr key={nomination.NominationID}>
                            <td>
                              <div className="d-flex align-items-center">
                                ...
                                <div>
                                  <div className="fw-semibold">{nominee?.FullName || "Unknown User"}</div>
                                  <small className="text-muted">{nominee?.Email || "No email"}</small>
                                </div>
                              </div>
                            </td>
                            <td>
                              <div className="d-flex align-items-center">
                                <div className="avatar me-2">
                                  {nominator?.ProfilePhoto ? (
                                    ...
                                    <>{nominator?.FullName.charAt(0).toUpperCase() || 'U'}</>
                                  )}
                                </div>
                                <div>
                                  <div className="fw-semibold">{nominator?.FullName || "Unknown User"}</div>
                                  <small className="text-muted">{nominator?.Email || "No email"}</small>
                                </div>
                              </div>
                            </td>
                            <td className="text-center">
                              <span className={`badge nomination-status-badge ${
                                nomination.Status === "Accepted" ? "bg-success" :
                                nomination.Status === "Rejected" ? "bg-danger" : "bg-warning"
                              }`}>
                                {nomination.Status}
                              </span>
                            </td>
                            <td className="text-center">
                              <span className="badge bg-primary">{nomination.VoteCount}</span>
                            </td>
                            <td className="text-center">
                              <small className="text-muted">{formatDate(nomination.NominatedAt)}</small>
                            </td>
                            <td className="text-center">
                              <button
              ...
              <div>
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h4 className="fw-semibold">
                    ...
                    Recent Votes
                    <span className="badge bg-primary ms-2">{votes.length}</span>
                  </h4>
                </div>
                      ...
                      {votes.map((vote) => {
                        ...
                        return (
                          <tr key={vote.VoteID}>
                              ...
                              <div className="d-flex align-items-center">
                                <div className="avatar me-2">
                                  {voter?.ProfilePhoto ? (
                                    ...
                                    <>{voter?.FullName.charAt(0).toUpperCase() || 'U'}</>
                                  )}
                                </div>
                                <div>
                                  <div className="fw-semibold">{voter?.FullName || "Unknown User"}</div>
                                  <small className="text-muted">{voter?.Email || "No email"}</small>
                                </div>
                              </div>
                            </td>
                            <td>
                            ...
                            <td>
                              <small className="text-muted">{formatDate(vote.VotedAt)}</small>
                            </td>
                          </tr>
                        );
                      })}
